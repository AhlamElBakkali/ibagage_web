...
@Component({
  ...
  styleUrl: './arrival-map.component.scss'
})
export class ArrivalMapComponent extends CustomerComposant implements AfterViewInit {
  ...
  async ngAfterViewInit() {
    this.vars.data = await this.getDataObject('annonce.new');
  ...
  getAddressFromCoordinates(latitude: number, longitude: number) {
    this.backendService.customer().getAddress(latitude, longitude).subscribe(
      (response: any) => {
        this.vars.adresseArrival = response.address.neighbourhood || 'Route sans nom';
        this.backendService.customer().displayAddress(this.vars.adresseArrival, 'adresseArrival');
      },
      (error) => {
